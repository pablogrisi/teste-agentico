# Codex/GPT e agentes compatíveis — LicIA Analisadora
...
Toda feature de produto passa por um ciclo de seis papéis — PM, Engenheiro, Dev, Agente de Testes, Crítico, Documentador —, cada um com um ponto de aprovação humana antes do próximo agir. Ver `.ai-dev/agent-roles.md` e `.ai-dev/workflow.md` para o ciclo completo. Se esta sessão suportar subagentes customizados do Codex CLI, as definições de cada papel estão em `.codex/agents/*.toml` (ver `.ai-dev/agents/README.md` — trate como experimental, com fallback para persona simulada se o subagente não for reconhecido).
