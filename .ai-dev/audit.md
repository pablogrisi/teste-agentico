...
- **PM + Engenheiro** (`27c75dc`): User Story do recorte frontend no `roadmap.md` (RF-016) + `docs/engineering/specs/021-relatorio-pdf-frontend.tsd.md`.
- **Dev** (`a142d5c`):
  - `analises-gateway.ts`: `AnalisesGateway.urlRelatorio(analiseId: string): string | null` na interface (docstring → `GET /analises/:id/relatorio`, TSD-011; `null` sem backend).
  - `http-analises-gateway.ts`: `urlRelatorio(id)` → `` `${this.baseUrl.replace(/\/+$/, "")}/analises/${encodeURIComponent(id)}/relatorio` `` — mesmo padrão do `urlPdf` do RF-014 (URL de navegação, não `fetch`).
  - `fixtures-analises-gateway.ts`: `urlRelatorio(_analiseId)` → `null`.
  - `BaixarRelatorio.tsx` + `.module.css` (novo, client): `const url = getAnalisesGateway().urlRelatorio(analiseId)`; `url` → `<a className={styles.botao} href={url} target="_blank" rel="noopener noreferrer">Baixar relatório</a>`; `url === null` → `<span.raiz><span.botaoOff aria-disabled="true">Baixar relatório</span><span.aviso role="note">Disponível com a tela conectada ao backend.</span></span>`. CSS reaproveita o vocabulário do botão do `ConcluirAnalise` (RF-012): `.botao` fundo `--color-brand-default` / texto `--color-text-inverse` / `radius-8` / `hover` `--color-brand-hover`; `.botaoOff` em `--color-icon-disabled`.
  - `TelaAnalise.tsx`: `acao` vira ternário de 3 casos — `PRONTA_PARA_REVISAO` → `<ConcluirAnalise>` (RF-012), `CONCLUIDA` → `<BaixarRelatorio analiseId={detalheEfetivo.id} />`, senão `undefined`. `AnaliseHeader` intocado. Concluir pelo modal do RF-012 troca "Concluir análise" → "Baixar relatório" sem reload (o `detalheEfetivo` já existe do RF-012).
- **Testes (mecânico)** (`f431ca1` + ajuste do Crítico em `512f9e0`): `npm run ci` ✅ — `eslint .` + prettier, `tsc --noEmit`, **vitest 247 → 250/250 (26 arquivos)**, `next build` (rota `/analise/[id]` ƒ dynamic). Novos/ampliados: `http-analises-gateway` (contrato de `urlRelatorio` — `${base}/analises/a%201/relatorio` a partir de `${BASE}/`, id encodado + sem barra dupla), `fixtures-analises-gateway` (`urlRelatorio` → `null`), `baixar-relatorio.test.tsx` (novo — com `urlRelatorio` mockado → `getByRole("link")` com `href`/`target=_blank`/`rel~noopener`, sem `role="note"`; `null` → texto `aria-disabled` + `role="note"`, sem link), `tela-analise` (CONCLUIDA mostra "Baixar relatório" e não "Concluir análise"; PRONTA o contrário; `it.each` de PENDENTE/PROCESSANDO/ERRO_PROCESSAMENTO → nenhum dos dois; link com backend; e o teste de conclusão do RF-012 estendido para asserir "Baixar relatório" após concluir pelo modal).
- **Smoke conjunto contra o backend real** — NestJS + PostgreSQL 17 (`:3000`) + frontend buildado com `NEXT_PUBLIC_API_BASE_URL` (`:3201`). Contra as análises do smoke do RF-012: análise `CONCLUIDA` → o cabeçalho mostra "Baixar relatório" (`href = http://localhost:3000/analises/<id>/relatorio`, `target="_blank"`, `rel` com `noopener`); clicar abre **nova aba** em `GET /analises/:id/relatorio` → `200`, `Content-Type: application/pdf`, `Content-Disposition: inline; filename="relatorio-analise-<id>.pdf"`, corpo `%PDF-` — o visor nativo renderiza o "Relatório de Análise de Conformidade" (NUP, objeto, responsável, datas, resumo, requisitos por área/norma/parecer). Análise `PRONTA_PARA_REVISAO` → "Concluir análise", **sem** "Baixar relatório". `GET /analises/:id/relatorio` numa análise não `CONCLUIDA` → `409` (confirmado direto na API — a UI nem oferece o botão). Modo fixtures (`:3202`, sem `NEXT_PUBLIC_API_BASE_URL`): botão "Baixar relatório" **desabilitado** (cinza) + aviso "Disponível com a tela conectada ao backend.".
- **Crítico** (subagente `critico`): **APROVADO**, sem bloqueadores. Ajuste após o Crítico (`512f9e0`): teste `it.each` dos estados sem botão (`PENDENTE`/`PROCESSANDO`/`ERRO_PROCESSAMENTO`) — fecha o critério de aceite §8 item 2. Não-bloqueador de follow-up: A11y unificada nos três botões desabilitados (`BaixarRelatorio` / `ConcluirAnalise` travado / `AnaliseVisor` sem backend) — `aria-describedby` ligando o pseudo-botão ao `role="note"`; fora do escopo desta slice.
- **Documentador**: TSD-021 (`status: implementada`, §8 todos os itens marcados, §10 REF-11 inspecionado, frontmatter atualizado), `roadmap.md` (RF-016 frontend → `implementada — Crítico ✅`; tabela de sequência linha 12; seção RF-016 → Status + "Notas do ciclo (frontend — 02/09/2026)"; **marco "MVP frontend completo"** após a seção RF-016), checkpoint §1 (marco + `main`), §2 (nenhuma spec ativa nas duas frentes), §3.1 (Crítico + Documentador RF-016), §5, §7, este arquivo, `frontend/README.md`.

Evidência visual: `frontend/docs/visual-reference/rf-016/` — `rf016-botao-concluida.png` (botão no cabeçalho da análise concluída, contra o backend real), `rf016-relatorio-aberto.png` (o relatório PDF renderizado na nova aba), `rf016-sem-backend.png` (botão desabilitado + aviso, modo fixtures) + `README.md` (REF-11; divergências: afordância inexistente no protótipo, abre em nova aba, sem backend desabilitado + aviso).

Estado: ciclo fechado com Crítico ✅, `npm run ci` verde (250 testes), smoke conjunto contra o NestJS real feito. **Aguardando push da branch `frontend/rf-016-relatorio` + merge na `main`** (depois de RF-012). **Com RF-012 e RF-016 mergeados, o MVP frontend está completo — não há próximo RF de frontend.**

Decisões: link `<a target="_blank">` para `GET /analises/:id/relatorio` (não fetch+blob+download forçado) — validada pelo usuário; botão só em `status === "CONCLUIDA"`, no slot `acao` do "Concluir análise"; sem backend → desabilitado + aviso (não escondido); `urlRelatorio` no gateway (composição de URL no data layer, ao lado do `urlPdf`).

Pendências: push + merge de RF-012 e RF-016; acabamento (§5 do checkpoint — A11y unificada dos botões desabilitados, re-sync `?requisitos=` no back/forward, contraste AA, brasão oficial); smoke de `/` + "Nova análise" contra o backend real; a integração da IA real continua sendo o único item de backend.
