...
- **Frontend: RF-008 / TSD-016 na branch `frontend/rf-008-parecer`** — implementada, Crítico aprovou, `npm run ci` verde (164 testes). Aguardando push da branch + merge na `main`. Próximo ciclo de frontend: **RF-011** (marcar "verificado" interativo) — PM apresenta a User Story + TSD para validação antes de implementar. (RF-009/TSD-015 já mergeada em `2e8f6d5`.)
...
**Documentador (01/09/2026):** ciclo RF-012/013/015/TSD-010 **fechado, aguardando merge** (branch `backend/rf-012-conclusao`). SDD §7 ("Concluir análise" — contrato final), §8 (`analise`: `analista_id`/`concluida_em` expostos, transição condicional). `questoes-abertas.md` P-01/P-02 anotadas (conclusão interna, auditoria mínima). Roadmap RF-012/013/015 → `implementada` (branch) + notas do ciclo. Detalhe em `.ai-dev/audit.md` (`01/09/2026`).
...
- [x] RF-008 / TSD-016 (frontend) — modal "Alterar parecer" na branch `frontend/rf-008-parecer` (01/09/2026); Crítico ✅; `npm run ci` verde (164 testes).
- [ ] **Frontend:** push da branch `frontend/rf-008-parecer` + merge na `main`.
...
2. **Frontend (Vinicius):** RF-010/TSD-013 (`28c9e8c`), RF-007/TSD-014 (`cbe9f5b`) e RF-009/TSD-015 (`2e8f6d5`) mergeadas. RF-008/TSD-016 (modal "Alterar parecer") implementada, **Crítico aprovou** na branch `frontend/rf-008-parecer`; falta o **push da branch + merge na `main`**. Depois, **RF-011** (marcar "verificado" interativo), linha 8 da tabela de sequenciamento do frontend. O PM apresenta a User Story + a TSD para validação **antes** de implementar; quando o RF fechar, push da branch + merge + push da `main`.
