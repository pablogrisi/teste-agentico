...
- **RF-001 + RF-004 / TSD-011** (`frontend/`) — **implementada** na branch `frontend/rf-001-nova-analise` (a partir de `frontend/rf-002-listagem`); aguardando revisão + merge. Modal "Nova análise" (NUP/objeto/upload de PDF), validação client-side (obrigatórios + tipo PDF + 25 MB), `AnalisesGateway.criarAnalise` (multipart `POST /analises` no `HttpAnalisesGateway`) + teste de contrato; sucesso → navega para `/analise/[id]`; erro `422`/`5xx`/rede → banner com dados preservados. Branches frontend encadeadas: `tsd-002-fundacao` ← `rf-002-listagem` ← `rf-001-nova-analise`.
...
- **Frontend: RF-002 / TSD-010 implementada** na branch `frontend/rf-002-listagem` (01/09/2026, a partir de `frontend/tsd-002-fundacao`). Tela `/` real: `AnalisesTable` + `AnalisesToolbar` (busca debounced + chips de status) + `Paginator`, tudo na URL; `AnalisesListaVazia` (sem-analises / sem-resultado); `loading.tsx` + `error.tsx`. Camada de dados: `analises-query.ts` (parser tolerante), `status-analise.ts`, `HttpAnalisesGateway` (contrato TSD-005 + validação de formato), `getAnalisesGateway()` por `NEXT_PUBLIC_API_BASE_URL`; `FixturesAnalisesGateway` passou a aplicar busca/filtro/ordenação/paginação (24 fixtures). Migração `next lint` → `eslint .` (flat `eslint.config.mjs`, `@eslint/eslintrc`; `next build` com `eslint.ignoreDuringBuilds`). `npm run ci` verde. Evidência visual em `frontend/docs/visual-reference/rf-002/`. Aguardando revisão (Crítico) + merge.
- **Frontend: RF-001 + RF-004 / TSD-011 implementada** na branch `frontend/rf-001-nova-analise` (01/09/2026, a partir de `frontend/rf-002-listagem`). `NovaAnaliseButton` (substitui o botão desabilitado) + `NovaAnaliseModal` (client, `createPortal`): campos NUP (≤ 60) / Objeto (≤ 2000) / Arquivo (PDF, ≤ 25 MB); "Confirmar" só habilita quando válido; erro por campo (faixa do arquivo fica vermelha) e banner de erro de envio com os campos preservados (PRD §9). `src/lib/data/nova-analise.ts` (validações puras); `AnalisesGateway.criarAnalise` + `AnaliseValidacaoError`; `HttpAnalisesGateway.criarAnalise` = `POST {base}/analises` multipart, mapeia `201`, `422` (→ motivos), `5xx`/rede; `FixturesAnalisesGateway.criarAnalise` devolve análise sintética `PENDENTE` (andaime — não entra na lista). Sucesso → `router.push('/analise/<id>')`. `npm run ci` verde: `eslint .` + prettier, typecheck, **72 testes / 12 arquivos** (inclui contrato de `POST /analises`), build. Evidência visual em `frontend/docs/visual-reference/rf-001/`. Aguardando revisão (Crítico) + merge.
...
- [x] RF-001 + RF-004 / TSD-011 (frontend) — modal "Nova análise" + upload implementado na branch `frontend/rf-001-nova-analise` (01/09/2026); `npm run ci` verde (72 testes); aguardando revisão + merge.
- [x] Follow-up (frontend): `next lint` → `eslint .` (feito no ciclo de RF-002).
- [ ] **Frontend:** revisão (Crítico) + merge, na ordem das branches encadeadas: `frontend/tsd-002-fundacao` → `frontend/rf-002-listagem` → `frontend/rf-001-nova-analise` na `main`.
...
2. **Frontend (Vinicius):** TSD-002 (fundação), RF-002/TSD-010 (listagem) e RF-001+RF-004/TSD-011 (modal "Nova análise") implementadas em branches encadeadas (`frontend/tsd-002-fundacao` ← `frontend/rf-002-listagem` ← `frontend/rf-001-nova-analise`), todas com `npm run ci` verde. Próximo: revisão (Crítico) das três + merge na `main` nessa ordem. Depois, 3º ciclo de RF do frontend — **RF-010** (tela de análise: abas Checklist/Técnica + navegação), linha 4 da tabela de sequenciamento do frontend (o backend de RF-010 já saiu em RF-009/TSD-007).
