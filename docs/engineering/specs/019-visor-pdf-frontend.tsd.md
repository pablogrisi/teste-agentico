...
| Smoke/validação manual contra dependência externa real | **Feito** (02/09/2026) | NestJS + PostgreSQL 17 do sistema em `:3000`, frontend com `NEXT_PUBLIC_API_BASE_URL` em `:3201`. `POST /analises` (PDF de 4 páginas) → processa → `totalPaginasPdf=4`; o `<iframe>` carrega `GET /analises/:id/pdf` (`inline`) e renderiza; `PATCH { paginaReferencia: 3 }` do editor grava no banco sem R-06; clicar na referência move o visor para `#page=3`; preflight CORS `204`. Evidência: `frontend/docs/visual-reference/rf-014/rf014-real-*.png`. |
