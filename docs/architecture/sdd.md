...
- **Integrações:**
  1. **Capacidade de análise assistida por IA** — recebe o PDF + a base de requisitos e devolve, por requisito, status sugerido e página. O projeto **aciona** um serviço externo atrás da porta `AnaliseIaPort`: adapter `stub` (padrão) ou `AnaliseIaOpenAiAdapter` (OpenAI/GPT via SDK `openai`, `IA_ADAPTER=openai`). Contrato fechado no ciclo A-02 / TSD-022 (`questoes-abertas.md` R-07).
...
- `AnaliseIaPort` com dois adapters: **stub** (determinístico, para dev/teste, padrão) e **`AnaliseIaOpenAiAdapter`** (OpenAI/GPT via SDK `openai`, selecionado por `IA_ADAPTER=openai`; TSD-022 — A-02 fechada).
...
A capacidade de IA entra como **porta** (`AnaliseIaPort`) com adapters trocáveis por configuração. Isso permitiu desenvolver, testar e rodar o MVP antes de o contrato externo estar fechado, e isola instabilidade/custo da IA do resto do sistema. Com A-02 fechada (TSD-022), troca-se do `stub` para o `AnaliseIaOpenAiAdapter` por `IA_ADAPTER=openai` — sem tocar no worker nem na porta. O `AnaliseIaOpenAiAdapter` decide **uma vez por análise** entre dois caminhos internos (TSD-023): o **primário** extrai o texto do PDF **por página** no servidor (`pdfjs-dist`), monta um bloco com marcadores `=== Página N ===` e manda esse **texto** ao modelo como `input_text` — sem subir o binário; o **fallback automático** (quando a extração rende pouco/nenhum texto — PDF escaneado) sobe o PDF pela Files API (`input_file`), como na TSD-022. A troca de caminho é interna ao adapter — o worker e a porta não sabem qual foi usado.
...
| AnaliseIaPort | Contrato interno: (PDF + requisitos) → lista de (requisito, status sugerido, página) | StubAdapter determinístico (padrão) + `AnaliseIaOpenAiAdapter` (OpenAI/GPT, `IA_ADAPTER=openai`, TSD-022); alvo de teste de contrato. O adapter OpenAI manda ao modelo o **texto do PDF por página** (marcadores `=== Página N ===`, `input_text`) e cai para a **Files API (`input_file`)** quando a extração de texto é insuficiente — decisão interna 1×/análise (TSD-023) |
...
| `AnaliseIaPort` (interno) | `analisar(pdf, requisitos[]) → { requisitoId, statusSugerido, paginaReferencia? }[]` | Forma estável; A-02 fechada — `AnaliseIaOpenAiAdapter` (TSD-022 + TSD-023) mapeia `codigo → requisitoId` a partir da saída `json_schema` `strict` da OpenAI. Entrada ao modelo: **texto do PDF por página** (`=== Página N ===`) como `input_text` no caminho primário; **PDF pela Files API** (`input_file`) no fallback automático para PDF sem texto aproveitável (TSD-023). `paginaReferencia` no caminho primário = número `N` do marcador de página do texto extraído. Alvo de teste de contrato (hoje contra o stub; adapter OpenAI com unit + SDK e `pdfjs-dist` mockados) |
...
| Capacidade de IA atrás de `AnaliseIaPort` com stub (padrão) + `AnaliseIaOpenAiAdapter` (OpenAI/GPT, `IA_ADAPTER=openai`) | A-02 fechada no ciclo A-02 / TSD-022; IA é cara/instável, e o stub sustentou o MVP | Permitiu MVP e testes sem a IA real; risco residual stub↔OpenAI real até o smoke ponta-a-ponta com a `OPENAI_API_KEY` (plano B: `chat.completions` + `response_format`) |
| Adapter OpenAI manda **texto do PDF por página** ao modelo, com **fallback automático** para a Files API (TSD-023) | O smoke real de 02/09/2026 recusou um edital de 142 páginas / 13,8 MB (texto nativo) com `400 input exceeds the context window`; mandar o binário a cada lote também multiplica o custo em tokens | Editais grandes passam a caber no contexto e o custo cai; PDF escaneado ainda é analisável pelo fallback. Decisão interna 1×/análise, sem env var (limiar fixo `MIN_MEDIA_CHARS_POR_PAGINA=100`). Nova dep `pdfjs-dist@3.11.174` (plano B — `unpdf`/pdfjs v4 ESM-only sob `ts-jest` CJS) traz a **CVE-2024-4367** (high), mitigada (`isEvalSupported: false` + hardening de fonte + sem rendering) e **aceita pelo usuário como dívida rastreada** (02/09/2026); follow-up = migrar para pdfjs v4+/`unpdf` após o `ts-jest` ir para ESM e re-rodar `npm audit` |
...
- [x] Teste de contrato da `AnaliseIaPort` (forma de entrada/saída) que o adapter real precisa satisfazer. (Roda hoje contra o stub; `AnaliseIaOpenAiAdapter` tem unit com o SDK `openai` mockado — TSD-022.)
...
- [ ] Validação manual/smoke contra a capacidade de IA real (custo/latência/comportamento) — não substitui os testes acima. Contrato fechado (TSD-022); o smoke com a `OPENAI_API_KEY` real segue **pendente** da chave.
...
- [x] A-02 — contrato da capacidade de IA (campos, auth, timeout, retry; aciona vs. consome) — **fechada** no ciclo A-02 / TSD-022 (+ TSD-023, que trocou a entrada ao modelo para texto por página com fallback para a Files API): o projeto **aciona** a OpenAI/GPT via SDK `openai` (ver `questoes-abertas.md` R-07). Pendente só o smoke real com a chave.
