...
| AnaliseIaPort | Contrato interno: (PDF + requisitos) → lista de (requisito, status sugerido, página) | StubAdapter determinístico (padrão) + `AnaliseIaOpenAiAdapter` (OpenAI/GPT, `IA_ADAPTER=openai`, TSD-022); alvo de teste de contrato |
...
| `AnaliseIaPort` (interno) | `analisar(pdf, requisitos[]) → { requisitoId, statusSugerido, paginaReferencia? }[]` | Forma estável; A-02 fechada — `AnaliseIaOpenAiAdapter` (TSD-022) mapeia `codigo → requisitoId` a partir da saída `json_schema` `strict` da OpenAI. Alvo de teste de contrato (hoje contra o stub; adapter OpenAI com unit + SDK mockado) |
...
- [x] A-02 — contrato da capacidade de IA (campos, auth, timeout, retry; aciona vs. consome) — **fechada** no ciclo A-02 / TSD-022: o projeto **aciona** a OpenAI/GPT via SDK `openai` (ver `questoes-abertas.md` R-07). Pendente só o smoke real com a chave.
