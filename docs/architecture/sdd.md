...
| Worker de processamento | Selecionar análises `PENDENTE`, chamar `AnaliseIaPort`, gravar sugestões e páginas, avançar status; re-selecionar presas no boot | Em processo, single-flight; sem Redis no MVP (A-06) |
...
| `requisito` | Item da base fixa de verificação | Campos mínimos: `id`, `codigo`, `area` (CHECKLIST \| TECNICA), `titulo`, `descricao`, `norma_referencia`, `obrigatorio` (bool), `ordem`, `ativo`. Estrutura definitiva: A-03; conteúdo do 1º subconjunto: P-07. Seed versionado |
